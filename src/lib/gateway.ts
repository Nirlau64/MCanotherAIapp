/**
 * Browser WebSocket client for the Hermes JSON-RPC gateway.
 *
 * Adapted from web/src/lib/gatewayClient.ts for the Chat PWA.
 *
...
export class GatewayClient {
  ...
  onState(cb: (s: ConnectionState) => void): () => void {
    ...
    cb(this._state);
    return () => void this.stateListeners.delete(cb);
  }
  ...
  on<P = unknown>(
    ...
    return () => void set!.delete(cb as (ev: GatewayEvent) => void);
  }
  ...
  async connect(): Promise<void> {
    ...
    const ws = new WebSocket(
      `${scheme}//${location.host}/api/ws?${authName}=${encodeURIComponent(authValue)}`,
    );
    this.ws = ws;
    ...
    // after the open promise resolves can race past it.
